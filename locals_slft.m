function [original_local_start_time,original_local_end_time] = locals_slft(num_j,R, r,d,LFT,forestset,ad)
global Simul_Num resource_cate T

LLFT = zeros(1,num_j);
for lie = 1:num_j
    LLFT(1,lie) = ceil(sum(LFT(:,lie))/Simul_Num);  %取每一列的均值
end
%当LFT一样时，按照min（LFT）为活动分配资源
[A,index] = sort(LLFT);  %LFT升序排序 index指对应的活动序号,
