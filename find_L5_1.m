function [L5,L4] = find_L5_1(LP,GlobalSourceRequest,skill_cate,skill_count,iter_skill_num)
...
if count_1~=size(LP,1)  %当前存在项目满足某一技能可用量
 ...
 %  rand('seed', 4);
    %  L4 = randperm(length(order_x));%随机给出一个顺序如1，3
...
if count_1~=0       %~isempty (not_satisfy_pro)  %如果空集无法缩为一行了
    ...
    iter_not = cell(1,size(not_satisfy_pro,1)*length(not_satisfy_pro));
    for   hang_x = 1:size(not_satisfy_pro,1)
        iter_not(1,(hang_x-1)*length(not_satisfy_pro)+1:hang_x*length(not_satisfy_pro)) = not_satisfy_pro(hang_x,:);
    end
end
%%
